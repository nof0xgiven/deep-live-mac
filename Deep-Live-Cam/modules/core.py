...
def assert_video_resource_limits(width: int, height: int, frame_count: int) -> None:
    if width <= 0 or height <= 0:
        raise ValueError(f"Invalid target video dimensions: {width}x{height}.")
    pixels = width * height
    if pixels > MAX_VIDEO_PIXELS:
        raise ValueError(
            f"Target video dimensions {width}x{height} exceed the "
            f"{MAX_VIDEO_PIXELS} pixel limit."
        )
    if frame_count > MAX_VIDEO_FRAMES:
        raise ValueError(
            f"Target video frame count {frame_count} exceeds the "
            f"{MAX_VIDEO_FRAMES} frame limit."
        )
...
def limit_resources() -> None:
    ...
    if modules.globals.max_memory:
        ...
        if platform.system().lower() == 'windows':
            ...
            # macOS rejects lowering RLIMIT_DATA from its inherited infinity value.
            return
        else:
            import resource
...
def start() -> None:
    ...
    try:
        width, height = get_video_dimensions(modules.globals.target_path)
        frame_count = estimate_frame_count(modules.globals.target_path, fps)
        assert_video_resource_limits(width, height, frame_count)
    except ValueError as error:
        update_status(str(error))
        return
    except Exception as error:
        update_status(f"Unable to inspect target video resource usage: {error}")
        return
