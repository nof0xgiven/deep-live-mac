...
RANDOM_FACE_URL = "https://thispersondoesnotexist.com/"
MAX_RANDOM_FACE_BYTES = 5 * 1024 * 1024
MAX_RANDOM_FACE_PIXELS = 20_000_000
...
def download_random_face_image(
    url: str = RANDOM_FACE_URL,
    destination_dir: Optional[str] = None,
    max_bytes: int = MAX_RANDOM_FACE_BYTES,
    max_pixels: int = MAX_RANDOM_FACE_PIXELS,
) -> str:
    if max_bytes <= 0:
        raise ValueError("Random face download byte limit must be positive.")
    if max_pixels <= 0:
        raise ValueError("Random face image pixel limit must be positive.")

    output_dir = destination_dir or tempfile.gettempdir()
    os.makedirs(output_dir, exist_ok=True)
    fd, temp_path = tempfile.mkstemp(
        prefix="deep_live_cam_random_face_",
        suffix=".jpg",
        dir=output_dir,
    )
    os.close(fd)
    ...
    try:
        with requests.get(
            ...
            if content_length:
                try:
                    ...
                    raise ValueError("Random face response has invalid Content-Length.") from error
                if declared_size > max_bytes:
                    raise ValueError(
                        f"Random face response exceeds {max_bytes} byte limit."
                    )
        ...
        with Image.open(temp_path) as image:
            ...
            if width * height > max_pixels:
                raise ValueError(
                    f"Random face image exceeds {max_pixels} pixel limit."
                )
            image.verify()
...
class MainWindow(QMainWindow):
    ...
    def _on_random_face(self) -> None:
        if _PREVIEW is not None:
            _PREVIEW.hide()
        try:
            temp_path = download_random_face_image()
