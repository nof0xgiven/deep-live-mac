...
def _decompose_reflect_pad(model) -> bool:
    ...
    for node in reflect_pads:
        ...
        if len(pads) < 8:
            raise ValueError(f"Unsupported reflect Pad initializer length: {len(pads)}")
        spatial_pads = [int(pads[index]) for index in (2, 3, 6, 7)]
        if any(pad < 0 for pad in spatial_pads):
            raise ValueError("Reflect Pad rewrite does not support negative pads.")
        if int(pads[0]) or int(pads[1]) or int(pads[4]) or int(pads[5]):
            raise ValueError("Reflect Pad rewrite only supports spatial padding.")
        if int(pads[2]) != int(pads[6]) or int(pads[3]) != int(pads[7]):
            raise ValueError("Reflect Pad rewrite only supports symmetric spatial padding.")
        max_pad = max(max_pad, *spatial_pads)

    if max_pad > MAX_REFLECT_PAD:
        raise ValueError(
            f"Reflect Pad rewrite refuses pad size {max_pad}; "
            f"maximum supported pad is {MAX_REFLECT_PAD}."
        )
